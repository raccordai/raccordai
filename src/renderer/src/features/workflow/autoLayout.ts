...
/**
 * Computes tidy positions for a React Flow graph, arranged as a strict grid:
...
export function autoLayoutPositions(
  ...
  const flowExtent = (n: Node) => (direction === 'LR' ? nodeSize(n).width : nodeSize(n).height)
  const seqExtent = (n: Node) => (direction === 'LR' ? nodeSize(n).height : nodeSize(n).width)
  ...
  const out: Array<{ id: string; position: { x: number; y: number } }> = []
...
function shotNumber(n: Node): number | undefined {
  const doc = (n.data as { node?: { label?: unknown; key?: unknown } } | undefined)?.node
  for (const s of [doc?.label, doc?.key]) {
    if (typeof s === 'string') {
      const m = s.match(/\d+/)
 ...
 */
function sequenceIndex(nodes: Node[]): Map<string, number> {
  const createdAt = (n: Node): number | undefined => {
    const c = (n.data as { node?: { createdAt?: unknown } } | undefined)?.node?.createdAt
 ...
 */
function computeRows(nodes: Node[], seq: Map<string, number>): Map<string, number> {
...
/** Real rendered size when React Flow has measured the node, else the approximation. */
