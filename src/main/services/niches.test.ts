...
describe('transcripts', () => {
  ...
  it('still attempts videos the API flags captionless (the flag misses ASR tracks)', async () => {
    const { nicheId } = await seedNicheWithChannel()
    vi.mocked(fetchChannelsByIds).mockResolvedValue([CHANNEL])
    vi.mocked(fetchUploads).mockResolvedValue([
      { videoId: 'vid1', publishedAt: null },
    ...
    vi.mocked(fetchVideosMeta).mockResolvedValue([
      ...
      // The Data API's `caption` flag only reflects MANUAL tracks — an
      // ASR-only video reports false but its transcript fetches fine.
    ...
    const attempted = vi.mocked(fetchTranscript).mock.calls.map(([id]) => id)
    expect(attempted).toContain('vid1')
    expect(attempted).toContain('nocaps00001')
  })
