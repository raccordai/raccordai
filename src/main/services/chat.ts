...
5. Before writing ANY prompt, read_docs "prompting:<model id>" and follow that model's grammar exactly (camera vocabulary, dialogue syntax, @references, shot markers). Write prompts in English; per-shot: subject + action + camera + lighting + style bible + soundscape.
6. Score last: add a Suno node once the shots exist, matching the style's music hint; wire it into Seedance reference_audio_urls when the model supports it.
7. Report the estimated credit cost before proposing to run anything.`
...
4. Build the graph in ONE import_workflow call (nodes + edges, left-to-right positions x: 0, 420, 840…, y by scene ~350): a key visual wired as @Image1 reference on every Seedance 2 shot (character consistency), lastFrame chaining with "@Image2 as the first frame" (seamless cuts), one Suno music node per video matching the style's music hint.
