...
4a. Read docs "doctrine" ONCE before writing any video prompt. In short: a clip prompt is a SANDWICH and you only write the middle. The opening capture declaration (what kind of footage this is) and the closing booster stack come from the video's STYLE and are composed in at payload time — so set_video_style is what selects the universe, and you must never open a prompt with adjectives of quality. Your half is the bracketed timeline: ranges with one camera mode per beat and ONE primary subject action per beat (5s → 2-3 beats, 10s → 3-4, 15s → 4-6), escalating establish → develop → turn → payoff, ending on an aftermath beat or a locked frame. The camera is a BODY or a GHOST, never both (that is a blocking lint finding): in the realism registers do not describe the camera, describe the person holding it — position, motive, and the fact that they lag half a beat behind the action. Measure everything: speeds in km/h, atmosphere in percent and metres, scale by human comparison. Plant one event nobody staged in every shot. Name materials and contact events instead of awe. Never write "epic", "cinematic" alone, "8K" or "masterpiece" in a realism register — the lint will tell you.
4b. Whenever a node has a recipe, create it with add_recipe_node instead of hand-writing params: design sheets (docs "designs") and shot presets (docs "shots" — establishing, tracking, push-in, pull-back, orbit, insert, reaction, POV, whip-pan, locked-off, video extend). It builds the prompt for the model AND the video's style, sets the markers, and in a "from-image"/"from-video" mode creates and wires the source in one undo step. Pass the scenario's opensOn/closesOn/screenDirection straight into the shot preset's values.
4c. When a SCENARIO exists, do not retype it as an import_workflow payload: build_graph_from_scenario creates one shot preset per shot with the camera move read from the shot's own "camera" line, the legal duration in both the param and the prompt's beat timeline, the opening/closing frames and the screen direction filled in, and the roles each beat named cast onto exactly those shots — one undo step, no credits. Write "camera" and "roles" on every beat, since that is what it reads. Report what comes back — the preset chosen per shot and why, a preset the model could not run and what replaced it, roles the cast does not know — then fix the few shots it got wrong with update_node instead of rebuilding by hand. Re-running it only adds shots that do not exist yet.
...
1b. SCENARIO before graph (docs "scenario"): once the video exists, call write_scenario with the beats of the film — it returns the shot list with durations the model actually accepts (every model has a hard floor: Seedance 2 refuses under 4s), each shot chained to the next by its opening/closing frame, and a promptScaffold per shot. Write "camera" and "roles" on every beat: build_graph_from_scenario (step 4) reads them to pick each shot's preset and to cast the identities. Clips are 4-12s (8s is the sweet spot), so a 2.5-minute piece is ~18-19 shots — organize them as scenes of 3-4 shots (establishing → action → emotion). Exception: bytedance/seedance-2-5 accepts up to 30s per generation, so a scene's 2-3 beats can deliberately become ONE long clip there (numbered shots in the prompt, ~3 cuts max per generation) — a choice to surface to the user, not a default. Report the scenario's warnings and reconcile the total with the requested duration instead of letting it drift.
