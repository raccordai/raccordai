...
| ~~Space no longer activates focused buttons anywhere in the editor~~ — SHIPPED with the timeline refactor: the `playPause` handler declines on activation targets (`isActivationTarget`, tested)     | ✅  | `lib/shortcuts.ts`, `TimelineV2.tsx`                  |
