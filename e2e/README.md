...
| `demo.e2e.mjs`           | demo mode (§9), external session: synthetic webm streamed through `demo:appendChunk` → transcode to mp4 → project video asset with the `demo_events` journal round-tripped; seq guard, double-start/stray-stop refusals; render bakes the automatic camera (PSNR proves the zoomed frame) and `demoCamera: false` opts out. |
