...
- **Image-input semantics — the #1 pitfall**: seedance-1.5 `input_urls`/grok `image_urls` are FRAME ANCHORS (the image appears on screen); seedance-2 `reference_*` are REFERENCES (guide only, invisible without a frame role in the prompt). Character sheets/storyboards → references ONLY. Details: `docs/models.md`; templates enforce it by test.
...
- **Auto-update** (`src/main/services/updater.ts`): electron-updater on the generic feed of `electron-builder.yml` (`https://updates.raccord.ai` — placeholder until releases ship). The packaged channel (`updateChannel` setting, stable|beta, Settings → Updates) drives BOTH the update feed and `getReleaseChannel()` (flag defaults). Dev builds: updater is a no-op ('unsupported').
